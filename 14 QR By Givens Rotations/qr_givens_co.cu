...
* nvcc -rdc=true -arch compute_35 qr_givens_co.cu -o gr
...
extern const int N = 4;		//this defines the number of elements in each vector
extern const int M = 4;		//this defines the number of vectors that need to be orthogonalized
...
__global__ void givens(double *A, double *Qt, double *R, double *G, int i) {
    int tid = threadIdx.x;
    int shift = 0;
    if(i >= N) {
        shift = i - (N - 1);
    ...
    double theta = atan( -(R[(tid + shift) * N + (tid + shift) + diff + tid] / R[(tid + shift) * N + (tid + shift)]) );
    int col = tid + shift;
    int row = ((tid + shift) * N + (tid + shift) + diff + tid) % N;
    //printf("Cos(theta) %f\n", cos(theta));
    //printf("Elements to annihilate: %f \n", R[(tid + shift) * N + (tid + shift) + diff + tid]);
    //printf("with: %f \n", R[(tid + shift) * N + (tid + shift) + diff + tid]);
...
int main( void ) {
	...
	for(int i = 1; i < 2 * (N - 1); i++){
        ...
        if (i < N) {
            ...
            nt = int(2 * (N - 1) - i) / int(2) + int(i % 2);
        }
        HANDLE_ERROR(cudaMemcpy(G, host_G, (N * N) * sizeof(double), cudaMemcpyHostToDevice));
		givens<<<1, nt>>>(dev_input, dev_Q_t, dev_R, G, i);
	}
